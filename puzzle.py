...
class Puzzle:
    def __init__(self, parent, logger, colorSpace, piece_w, piece_h, path, shuffle, displayInitial, displayEnd):
        super().__init__()
        self.parent = parent
    ...
    def build_pieces(self):
        img_array, height, width = self.jpg_image_to_array()
        pieces = [None for _ in range(self.piece_h * self.piece_w)]
        split_height = height // self.piece_h
        split_width = width // self.piece_w
        y = 0
        for i in range(self.piece_h):
            x = 0
            for j in range(self.piece_w):
                temp_img = img_array[y:y + split_height, x:x + split_width, :]
                pieces[i * self.piece_w + j] = temp_img
                x += split_width
            y += split_height
        img_array = pieces
        if self.shuffle:
    ...
    def populate_graph(self, results):
        graph = nx.Graph()
        for x in range(0, len(results)):
            for z in range(4):
                ...
                if not (graph.get_edge_data(x, match_x) is not None and graph.get_edge_data(x, match_x)[
                    'weight'] > match_score):
    ...
    def insert_failure(self, coord_arr, results):
        for i in range(20):
            fails = list(set(np.arange(self.piece_h * self.piece_w)) - set(coord_arr.ravel()))
            for fail in fails:
                ...
                try:
                    if position_1[0] == position_3[0] and (
                            position_1[1] - 1 == position_3[1] or position_1[1] - 1 == position_3[1] + 1):
                ...
                try:
                    if position_0[0] == position_3[0] and (
                            position_1[1] - 1 == position_3[1] or position_1[1] - 1 == position_3[1] + 1):
                        if coord_arr[position_1[0], position_3[1] + 1] != -1:
                            fails.append(coord_arr[position_1[0], position_3[1] + 1])
                        coord_arr[position_1[0], position_3[1] + 1] = fail
                ...
                try:
                    ...
                    print('Position is null')
                try:
                    if (position_0[1] == position_2[1]) and (
                            (position_0[0] + 1 == position_2[0] - 1) or position_0[0] + 1 == position_2[0] or
                            position_0[0] + 1 == position_2[0] - 2):
    ...
    def direct_comparison(self, shuffled_piece, original_piece, coordinates):
        ...
        self.logger.debug(str('RESULT of direct comparison: ' + str(count / self.piece_w * self.piece_h)))

    # Performs the neighbor_comparison measure. Returns the score for the given coordinates.
    def neighbor_comparison(self, shuffled_piece, original_piece, coordinates):
        shape_coord = coordinates.shape
        list_stacked = np.arange(self.piece_w * self.piece_h)
        list_stacked = list_stacked.reshape(self.piece_h, self.piece_w)
        w = shape_coord[1]
        h = shape_coord[0]
        counts = []
